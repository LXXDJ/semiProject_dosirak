...
import com.ohgiraffers.dosirak.user.review.common.Pagenation;
...
public class UserReviewService {
    ...
    public Map<String, Object> selectReviewList(Map<String, String> searchMap, int page, String userId) {
        ...
        int totalCount = userReviewMapper.selectTotalCount(searchMap, userId);
        log.info("reviewList totalCount : {}", totalCount);

        /* 2. 페이징 처리와 연관 된 값을 계산하여 SelectCriteria 타입의 객체에 담는다. */
        int limit = 10;         // 한 페이지에 보여줄 게시물의 수
        int buttonAmount = 5;   // 한 번에 보여질 페이징 버튼의 수
        SelectCriteria selectCriteria = Pagenation.getSelectCriteria(page, limit, buttonAmount, totalCount, searchMap, userId);
        log.info("selectCriteria : {}", selectCriteria);

        /* 3. 요청 페이지와 검색 기준에 맞는 게시글을 조회해온다. */
        List<ReviewDTO> reviewList = userReviewMapper.selectReviewList(selectCriteria);
        log.info("selectCriteria : {}", selectCriteria);
